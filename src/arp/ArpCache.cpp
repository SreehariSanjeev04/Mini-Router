 ...
 */
void ArpCache::put(const std::array<uint8_t, 4>& ip, const std::array<uint8_t, 6>& mac)
{
    cache[ip] = mac;
}
 ...
 */
std::optional<std::array<uint8_t, 6>> ArpCache::get(const std::array<uint8_t, 4>& ip) const
{
 ...
 */
bool ArpCache::contains(const std::array<uint8_t, 4>& ip) const
{
